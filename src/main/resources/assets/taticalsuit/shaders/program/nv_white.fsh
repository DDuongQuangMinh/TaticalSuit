...
void main()
{
	...
	/* STREAMING_CHUNK: Applying icy-white phosphor tint, grain, bloom, and vignette */
	// phosphor tint: pale cool blue/grey (White Phosphor)
	vec3 nv = luminance * vec3(0.38, 0.44, 0.50);
	...
	nv = mix(nv, vec3(0.85, 0.95, 1.0), bloom * 0.6);

	float overexposure = smoothstep(0.4, 0.9, luminance);
	nv = mix(nv, vec3(1.0, 1.0, 1.0), overexposure * 1.2);
